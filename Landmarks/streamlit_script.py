...
labels = 'Landmarks/landmarks_classifier_africa_V1_label_map.csv'
...
def run():
    ...
    img = PIL.Image.open('Landmarks/logo.png')
    img = img.resize((256,256))
    st.image(img)
    img_file = st.file_uploader("Choose your Image", type=['png', 'jpg'])
    if img_file is not None:
        save_image_path = './Landmarks/Uploaded_Images/' + img_file.name
